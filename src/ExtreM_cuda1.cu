...
struct OffsetComparator {
    ...
    __host__ __device__ bool operator()(int v, int u) const {
        ...
        return d_offset[v] > d_offset[u] || 
               ((fabs(d_offset[v] - d_offset[u]) == 0) && (v > u));
    }
};

struct CompareByData {
    const double* data;
    CompareByData(const double* _data) : data(_data) {}
...
void getdata(const std::string &filename, double *input_data_host, double *decp_data_host, double *decp_data_copy, 
    ...
    d_buffer[min_index1] = h_buffer[min_index1] - bound;
    d_buffer[max_index1] = h_buffer[max_index1] - bound;
...
__global__ void get_wrong_index_max(){
    ...
    int numMax = saddleTriplets[i*46 + 44];
    for(int p = 0; p<numMax; p++){
        if(saddleTriplets[p*46] != dec_saddleTriplets[p*46]){
...
__global__ void get_wrong_index_saddles(
    ...
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    // if(index == 0){
    //     for(int i=0;i<nSaddle2; i++) printf("%.17f\n", input_data[saddles2[i]]);
    // }
    if (index == 0 || index >= nSaddle2) return;
    if(decp_data[saddles2[index]] < decp_data[saddles2[index - 1]] || (decp_data[saddles2[index]] == decp_data[saddles2[index - 1]] && saddles2[index] < saddles2[index - 1])){
        int pos = atomicAdd(&wrong_saddle_counter, 1);
        // printf("%d %d %.17f %.17f %.17f %.17f\n", saddles2[index], saddles2[index - 1], decp_data[saddles2[index]], decp_data[saddles2[index - 1]], input_data[saddles2[index]], input_data[saddles2[index - 1]]);
        wrong_rank_saddle_index[pos * 2] = saddles2[index];
        wrong_rank_saddle_index[pos * 2 + 1] = saddles2[index - 1];
    }
    ...
    for (int i = start + 1; i < end; ++i) {
        int saddle = d_flattened_max2saddles[i];
        true_largest = d_flattened_max2saddles[i-1];
        true_largest_value = decp_data[true_largest];
        if(decp_data[saddle] < true_largest_value || (decp_data[saddle] == true_largest_value && saddle < true_largest)){
            // isFalse = true;
            // if(wrong_rank_saddle[maxId] == 0){
            int pos = atomicAdd(&wrong_saddle_counter, 1);
            wrong_rank_saddle_index[pos * 2] = saddle;
            wrong_rank_saddle_index[pos * 2 + 1] = true_largest;
            // wrong_rank_saddle[maxId] = 1;
...
__global__ void get_wrong_index_saddles_join(
    ...
    // }
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index == 0 || index >= nSaddle1) return;
    if(decp_data[saddles1[index]] < decp_data[saddles1[index - 1]] || (decp_data[saddles1[index]] == decp_data[saddles1[index - 1]] && saddles1[index] < saddles1[index - 1])){
        int pos = atomicAdd(&wrong_saddle_counter_join, 1);
        wrong_rank_saddle_join_index[pos * 2] = saddles1[index];
        wrong_rank_saddle_join_index[pos * 2 + 1] = saddles1[index - 1];
    }
    ...
    for (int i = start + 1; i < end; ++i) {
        ...
        true_smallest = d_flattened_min1saddles[i-1];
        true_smallest_value = decp_data[true_smallest];
        if(decp_data[saddle] < true_smallest_value || (decp_data[saddle] == true_smallest_value && saddle < true_smallest)){
        // if(isless_shared(saddle, true_smallest, decp_data[saddle], true_smallest_value)){
            // isFalse = true;
            // printf("%d %d %.17f %.17f\n", true_smallest, saddle, true_smallest_value, decp_data[saddle]);
            // if(wrong_rank_saddle_join[minId] == 0){
            int pos = atomicAdd(&wrong_saddle_counter_join, 1);
            wrong_rank_saddle_join_index[pos * 2] = saddle;
            wrong_rank_saddle_join_index[pos * 2 + 1] = true_smallest;
            // wrong_rank_saddle_join[minId] = 1;
...
void saddle_loops(dim3 gridSize_max, dim3 gridSize_2saddle, dim3 gridSize, dim3 blockSize){
    cudaMemcpyToSymbol(wrong_saddle_counter, &initialValue, sizeof(int));
    // init_saddle_rank_buffer<<<gridSize, blockSize>>>();
    get_wrong_index_saddles<<<gridSize_2saddle, blockSize>>>(d_max_offsets, d_flattened_max2saddles);
    init_delta<<<gridSize, blockSize>>>();
...
void saddle_loops_join(dim3 gridSize_min, dim3 gridSize_1saddle, dim3 gridSize, dim3 blockSize){
    cudaMemcpyToSymbol(wrong_saddle_counter_join, &initialValue, sizeof(int));
    // init_saddle_rank_buffer<<<gridSize, blockSize>>>(1);
    get_wrong_index_saddles_join<<<gridSize_1saddle, blockSize>>>(d_min_offsets, d_flattened_min1saddles);
    init_delta<<<gridSize, blockSize>>>();
...
void computeMax_saddle_table(std::vector<double> input_data){
    ...
    for (int i = 0; i < nMax_host; ++i) {
        std::sort(max2saddles[i].begin(), max2saddles[i].end(),
                  [&](int a, int b) { return input_data[a] < input_data[b] || (input_data[a] == input_data[b] && a < b); });
    }
    ...
    flattened_max2saddles_host.resize(max_offsets[nMax_host]);
    for(int i = 0; i < nMax_host; ++i) {
        std::copy(max2saddles[i].begin(), max2saddles[i].end(),
                flattened_max2saddles_host.begin() + max_offsets[i]);
    }
    ...
    // flattened数组
    std::cout<<"size: "<<flattened_max2saddles_host.size()<<std::endl;
    cudaMalloc(&d_flattened_max2saddles, flattened_max2saddles_host.size() * sizeof(int));
    ...
    cudaMemcpy(d_flattened_max2saddles, flattened_max2saddles_host.data(),
    flattened_max2saddles_host.size() * sizeof(int), cudaMemcpyHostToDevice);
...
void computeMin_saddle_table(std::vector<double> input_data){
...
void sort_saddle_range(int* saddles2_t, const double* data, int j) {
    thrust::device_ptr<int> saddles_ptr(saddles2_t);
    thrust::device_ptr<const double> data_ptr(data);

    thrust::sort(saddles_ptr, saddles_ptr + j,
        CompareByData(thrust::raw_pointer_cast(data_ptr)));
    cudaMemcpyToSymbol(saddles2, &saddles2_t, sizeof(int*));
    // std::vector<int> h_saddles(j);
    // std::vector<int> data1(num_Elements_host);
    // cudaMemcpy(h_saddles.data(), saddles2_t, j * sizeof(int), cudaMemcpyDeviceToHost);
    // cudaMemcpy(data1.data(), data, num_Elements_host * sizeof(double), cudaMemcpyDeviceToHost);
//     printf("Sorted saddle indices (with data values): %d\n", j);
//     for (int i = 0; i < j; ++i) {
//         int idx = h_saddles[i];
//         printf("  saddles[%d] = %d  (data[%d] = %.17f)\n", i, idx, idx, input_data_d[idx]);
//     }
}

void sort_saddle1_range(int* saddles2_t, const double* data, int j) {
    thrust::device_ptr<int> saddles_ptr(saddles2_t);
    thrust::device_ptr<const double> data_ptr(data);

    thrust::sort(saddles_ptr, saddles_ptr + j,
        CompareByData(thrust::raw_pointer_cast(data_ptr)));
    cudaMemcpyToSymbol(saddles1, &saddles2_t, sizeof(int*));
}
...
int main(int argc, char** argv){
    ...
    auto it = std::min_element(input_data_d.begin(), input_data_d.end());
    min_index1 = std::distance(input_data_d.begin(), it);

    std::cout << "Minimum value is " << *it << " at index " << min_index1 << std::endl;

    

    it = std::max_element(input_data_d.begin(), input_data_d.end());
    max_index1 = std::distance(input_data_d.begin(), it);

    std::cout << "maximum value is " << *it << " at index " <<  max_index1 << std::endl;
    ...
    // return 0;
    sort_saddle_range(saddles2_host, input_data_host, nSaddle2_host);
    sort_saddle1_range(saddles1_host, input_data_host, nSaddle1_host);
    ...
    // computeMax_saddle_table(input_data_d);
    // cudaDeviceSynchronize();
    // computeMin_saddle_table(input_data_d);
    // cudaDeviceSynchronize();
    ...
    if(host_number_of_false_cases == 0){
        ...
        if(host_wrong_max_counter == 0){
            saddle_loops(gridSize_max, gridSize_2saddle, gridSize, blockSize);
        }  
    }
    ...
    if(host_number_of_false_cases1 == 0 && preserve_join_tree){
        ...
        if(host_wrong_min_counter == 0){
            saddle_loops_join(gridSize_min, gridSize_1saddle, gridSize, blockSize);
        }
    ...
    while(host_number_of_false_cases > 0 || host_number_of_false_cases1 > 0|| host_count_f_max > 0 || host_count_f_min > 0 || host_count_f_saddle > 0 
        ...
        if(time_counter.size() == 7 || time_counter.size() == 6) {std::cout<<
        ...
        if(time_counter.size() == 7 || time_counter.size() == 6) std::cout<<"host_wrong neighbors: "<<host_number_of_false_cases<<", "<< host_number_of_false_cases1<<std::endl;
        if(host_number_of_false_cases == 0){
    ...
    cost(name, decp_d.data(), decp_data_copy_d.data(), input_data_d.data(),compressor_id,delta_counter_d);
