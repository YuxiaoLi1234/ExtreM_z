...
__device__ bool islarger_shared(const int v, const int u, float value_v1, float value_v2){
    return value_v1 > value_v2 || (fabs(value_v1 - value_v2)==0 && v > u);
}
...
__device__ void fix_saddle_local(int i, int *o_lowerStar_start, int o_size1){
    ...
    for (int d = 0; d < maxNeighbors; d++) {
        ...
        if (newX >= 0 && newX < width && newY >= 0 && newY < height && r >= 0 && r < width*height*depth && newZ<depth && newZ>=0) {
            ...
            if (neighbor_value < currentHeight || (fabs(neighbor_value - currentHeight)==0 && r < i)) {
...
__global__ void c_loop(int direction = 0){      
    ...
    if (direction == 0){
        ...
        if (vertex_type[index]!=4){
            ...
            //     return;
            // }
            if(decp_value>largest_value or (fabs(decp_value-largest_value)==0 and index>largest_index)){
                return;
            }
        ...
        if (vertex_type[index]!=0){
            ...
            //     return;
            // }
            if(decp_value>decp_smallest_value or (fabs(decp_value-decp_smallest_value)==0 and index>smallest_index)){
                return;
            }
...
__global__ void applyDeltaBuffer() {
    ...
    if (tid < num_Elements) {
        ...
        if(d_deltaBuffer[tid]!=-4.0 * bound){
            if(fabs(d_deltaBuffer[tid])>1e-6 && delta_counter[tid]<6 && fabs(input_data[tid]-(decp_data[tid] + d_deltaBuffer[tid])) < bound){
...
__global__ void classifyVertex_CUDA(int type = 0, int local = 1) {
    ...
    for (int d = 0; d < maxNeighbors; d++) {
        ...
        if (newX >= 0 && newX < width && newY >= 0 && newY < height && r >= 0 && r < width*height*depth && newZ<depth && newZ>=0) {
            ...
            if (neighbor_value < currentHeight || (fabs(neighbor_value - currentHeight)==0 && r < g_idx)) {
                if(type==0) lowerStars_t[vertexId*(maxNeighbors+1) + lowerCount] = r;
                binary[maxNeighbors - 1 - neighbor_size] = 0;
                lowerStar[lowerCount] = r;
                lowerCount++;
                
            } else if (neighbor_value > currentHeight || (fabs(neighbor_value - currentHeight)==0 && r > g_idx)) {
    ...
    if(type==1 && vertexId == 29197){
        printf("decp: %d %.7f %.7f %.7f\n", vertexId, input_data[vertexId], input_data[vertexId] - bound, decp_data[vertexId] );
        for(int q = 0; q<lowerCount; q++){
            int k = lowerStar[ q];
            printf("%d %.7f %.7f %.7f\n", k, input_data[k], input_data[k] - bound, decp_data[k]);
        }
        printf("ori:\n");
        int n = lowerStars[vertexId*(maxNeighbors+1) + maxNeighbors];
        for(int q = 0; q<n; q++){
            int k = lowerStars[vertexId*(maxNeighbors+1) + q];
            printf("%d %.7f %.7f %.7f\n", k, input_data[k] - input_data[vertexId], input_data[k] - bound - input_data[vertexId] + bound, decp_data[k] - decp_data[vertexId]);
            printf("%d %d %d %d\n", k, input_data[k] == input_data[vertexId], input_data[k] - bound == input_data[vertexId] - bound, decp_data[k] == decp_data[vertexId]);
        }
    }
    int decimal_value = 0;
...
void cost(std::string filename, float* decp_data, float* decp_data_copy, float* input_data, std::string compressor_id, std::vector<int> delta_counter){
        ...
        float bitRate = 32/overall_ratio; 
        ...
        outFile3 << std::setprecision(17)<<"BR: "<< 32/(float(original_indexSize)/compressed_dataSize) << std::endl;
